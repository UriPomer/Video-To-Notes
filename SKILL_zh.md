...
`--dry-run` 只是预览，不是完成条件。最终收尾顺序必须是：

1. 先验证 `notes.md` 里的每个图片引用都能在本地找到。
2. 运行 `prune_screenshots.py "<folder>" --dry-run` 并阅读预览。
3. 除非用户明确要求保留未引用截图，否则继续运行不带 `--dry-run` 的
   `prune_screenshots.py "<folder>"`。
4. 再次运行 `--dry-run`；必须显示 `unreferenced : 0` / `Nothing to prune`。

不要停在 dry-run。不要让 `screenshots/` 里残留 `notes.md` 没有引用的生成帧。
...
- [ ] **S8** — 终稿清理后，`prune_screenshots.py "<folder>" --dry-run` 显示未引用截图为 0。
